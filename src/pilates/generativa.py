...
def _parse_json_tolerante(raw: str) -> dict:
    """Parse JSON tolerante a markdown fences y truncamiento.
...
- Algo que ningún cluster mencionó
...
async def detectar_preguntas_huerfanas(resultados_g4: dict) -> dict:
    ...
    try:
        ...
        resultado = _parse_json_tolerante(raw)
    ...
    for ph in resultado.get("preguntas_huerfanas", []):
        try:
            async with pool.acquire() as conn:
                ...
                """, TENANT, ph.get("pregunta", ""),
                    ph.get("funcion_impactada", ""),
                    ph.get("lente_impactada", ""),
                    ph.get("int_que_la_haria", ""),
                    ph.get("prioridad", 3),
    ...
    try:
        ...
        await escribir(
            ...
            capa="generativa",
            estado="completado",
            detectando=f"{n} preguntas huérfanas",
            interpretacion=resultado["preguntas_huerfanas"][0]["pregunta"][:150] if n > 0 else "Ninguna",
            confianza=0.6,
            prioridad=5,
        )
    except Exception as e:
    ...
    if n > 0:
        try:
            ...
            await publicar("organismo_huerfanas", "question",
...
async def cristalizar_patrones() -> dict:
    ...
    if len(ejecuciones) < 3:
        return {"patrones_detectados": [],
                "razon": "Menos de 3 ejecuciones, insuficiente para cristalizar"}
    ...
    # Persistir cristalizaciones que requieren CR1
    ...
    if n > 0:
        try:
            from src.pilates.feed import publicar
            await publicar("organismo_cristalizador", "diamond",
...
async def consultar_sequito_para_enjambre(diagnostico_g4: dict,
                                           ints_en_duda: list[str]) -> dict:
    """Cuando el enjambre tiene INTs "en duda", consulta al séquito.

    Los 24 asesores (sequito.py) trabajan solo bajo demanda del cockpit.
    ...
    try:
        ...
        return {"consultados": 0, "error": "sequito.py no disponible"}

    resultados = {}
    for int_id in ints_en_duda[:3]:  # Máximo 3 consultas
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                resp = await client.post(
                    ...
                    json={
                        ...
                        "messages": [
                            {"role": "system",
                             "content": f"Eres {asesor['nombre']} ({int_id}). {asesor['angulo']}"},
