# B-ORG-R1: Redsys — Reemplazar Stripe por pagos reales via Caja Rural

**Fecha:** 23 marzo 2026
**Prioridad:** Alta (blocker para cobros reales)
**Dependencia:** Credenciales sandbox de Caja Rural (aún no recibidas)
**Acción:** Escribir TODO el código ahora. Testear cuando lleguen credenciales.

---

## CONTEXTO

stripe_pagos.py está implementado pero:
1. El Autófago lo detectó como obsoleto
2. Authentic Pilates usa Caja Rural de Navarra, no Stripe
3. La ficha técnica de Redsys ya está preparada (Ficha_Tecnica_Caja_Rural_Redsys.docx)

Redsys es la pasarela que usa Caja Rural. Soporta:
- Pago estándar por redirección (cliente → Redsys → pago → vuelta)
- COF (Credential on File) — cobro recurrente con token de tarjeta
- PayGold — enlace de pago por WhatsApp/email
- Bizum integrado en la pasarela

## ARCHIVO A CREAR

`src/pilates/redsys_pagos.py` — reemplaza `stripe_pagos.py`
...
REDSYS_SECRET_KEY       # Clave SHA-256 para firmar
REDSYS_ENVIRONMENT      # "test" o "production"
```

## URLs (ya definidas en ficha técnica)

```
Notificación:  /pilates/redsys/notificacion      (POST, Redsys → nosotros)
Retorno OK:    /pilates/redsys/retorno-ok         (GET, cliente vuelve tras pago OK)
Retorno KO:    /pilates/redsys/retorno-ko         (GET, cliente vuelve tras pago KO)
PayGold:       /pilates/redsys/paygold-retorno    (GET, retorno desde enlace PayGold)
```
...
## ENDPOINTS (añadir al router)

```python
# En router.py o en un nuevo redsys_router.py:

from fastapi import APIRouter, Request, Form
from fastapi.responses import HTMLResponse, RedirectResponse

redsys_router = APIRouter(prefix="/pilates/redsys", tags=["redsys"])

@redsys_router.post("/notificacion")
...
@redsys_router.get("/paygold-retorno")
...
(DES3 para diversificar la clave — estándar Redsys)

## TESTS

### TEST 1: Firma válida
```python
# Con clave de test de Redsys (pública):
# Sq9q0+RaOJGBiAn2mIWTIg== (clave sandbox estándar)
assert _sign(params_b64, "0001") is not None
assert len(_sign(params_b64, "0001")) > 20  # Base64 de HMAC
```

### TEST 2: Encode/decode params
```python
params = {"Ds_Merchant_Amount": "10050", "Ds_Merchant_Order": "0001"}
b64 = _encode_params(params)
decoded = _decode_params(b64)
assert decoded["Ds_Merchant_Amount"] == "10050"
```

### TEST 3: Notificación procesa pago OK
```
POST /pilates/redsys/notificacion → HTTP 200
```

### TEST 4: PayGold genera enlace
```
POST /pilates/redsys/paygold → returns order + payment_url (o error si no hay credenciales)
```
...
Este código está listo para desplegar. Cuando Caja Rural envíe las credenciales sandbox:
1. Añadir secrets a fly.io
2. Ejecutar migración 020_redsys.sql
3. Hacer 3 transacciones de prueba (estándar, COF inicial, COF recurrente)
4. Pedir paso a producción
