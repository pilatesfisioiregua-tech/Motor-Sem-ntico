# B-PIL-20c: Arquitecto de Presencia — Perfiles por Canal

**Fecha:** 2026-03-22
**Ejecutor:** Claude Code
**Dependencia:** B-PIL-20a + 20b desplegados (tablas om_voz_perfil_plantilla, om_voz_identidad, om_voz_irc, om_voz_pca, om_voz_estrategia existen con datos)
**Coste:** ~$0.01/canal (1 LLM call por canal)

---

## CONTEXTO

El Arquitecto de Presencia (B2.8 §3.2) diseña la **configuración completa** de cada perfil digital del negocio antes de que el Motor Tridimensional genere contenido. Es el escaparate antes de las estanterías.

Ningún software del mercado hace esto. Metricool/Hootsuite asumen que el perfil ya está bien. Nunca lo está.

El Arquitecto genera para cada canal:
- **Configuración automatizable** (texto de descripción, catálogo, Q&A) → se almacena como plantilla lista para aplicar
- **Instrucciones paso a paso** para lo que NO se puede automatizar (bio IG, highlights) → se envían a Jesús por WhatsApp como tarea asistida

Todo contextualizado por los 3 ejes: IRC (¿merece la pena este canal?), posición matricial (¿qué ángulo?), PCA (¿qué lenguaje/formato/tono?).

**Ya existe:** tabla `om_voz_perfil_plantilla` (creada en 20a). Este briefing la llena.

---

## FASE C1: Generador de perfiles por canal

**Archivo:** Crear `src/pilates/voz_arquitecto.py`
...
**Archivo:** `src/pilates/router.py` — AÑADIR al final, después de los endpoints de 20b.
...
```bash
# ── V1. Archivo creado ───────────────────────────────────────
ls src/pilates/voz_arquitecto.py
# PASS: archivo existe

# ── V2. Importación limpia ───────────────────────────────────
python -c "from src.pilates.voz_arquitecto import generar_perfil; print('OK')"
# PASS: sin errores

# ── V3. Generar perfil WhatsApp ──────────────────────────────
curl -X POST .../pilates/voz/perfil/whatsapp/generar
# PASS: status="ok"
# PASS: perfil.descripcion tiene max 256 chars
# PASS: perfil.descripcion NO contiene palabras prohibidas (fitness, gym, etc.)
# PASS: perfil.catalogo tiene al menos 3 servicios
# PASS: perfil.mensaje_bienvenida tiene max 1024 chars
# PASS: perfil.respuestas_rapidas tiene al menos 5 atajos
# PASS: perfil.instrucciones tiene pasos con "donde"
# PASS: plantillas_creadas >= 5

# ── V4. Generar perfil Google Business ───────────────────────
curl -X POST .../pilates/voz/perfil/google_business/generar
# PASS: status="ok"
# PASS: perfil.descripcion tiene max 750 chars
# PASS: perfil.descripcion contiene lenguaje de audiencia (dolor espalda, bienestar, etc.)
# PASS: perfil.qya_prellenadas tiene al menos 5 preguntas
# PASS: perfil.servicios tiene al menos 3 servicios
# PASS: perfil.posts_iniciales tiene al menos 2 posts

# ── V5. Generar perfil Instagram ─────────────────────────────
curl -X POST .../pilates/voz/perfil/instagram/generar
# PASS: status="ok"
# PASS: perfil.bio tiene max 150 chars
# PASS: perfil.highlights tiene al menos 4 highlights con orden
# PASS: perfil.primeros_posts tiene al menos 3 posts con hashtags

# ── V6. Generar perfil Facebook ──────────────────────────────
curl -X POST .../pilates/voz/perfil/facebook/generar
# PASS: status="ok"
# PASS: perfil.boton_cta tiene tipo y justificacion

# ── V7. Generar todos los perfiles ───────────────────────────
# NOTA: si V3-V6 ya crearon plantillas, este las acumula
curl -X POST .../pilates/voz/perfiles/generar
# PASS: status="ok" o "parcial"
# PASS: canales_generados >= 3 (WA, Google, IG tienen IRC >= 0.3)
# PASS: plantillas_total >= 15

# ── V8. Leer perfil de un canal ──────────────────────────────
curl .../pilates/voz/perfil/whatsapp
# PASS: canal="whatsapp"
# PASS: tipos incluye "descripcion", "catalogo", "mensaje_bienvenida", "respuestas_rapidas"
# PASS: cada plantilla tiene contenido, estado="borrador"

# ── V9. Aprobar plantilla ────────────────────────────────────
# ID=$(curl -s .../pilates/voz/perfil/whatsapp | jq -r '.plantillas.descripcion[0].id')
curl -X POST .../pilates/voz/plantilla/$ID/aprobar
# PASS: status="aprobado"

# ── V10. Marcar como aplicada ────────────────────────────────
curl -X POST .../pilates/voz/plantilla/$ID/aplicada
# PASS: status="aplicado"

# ── V11. Canal no soportado ──────────────────────────────────
curl -X POST .../pilates/voz/perfil/tiktok/generar
# PASS: error contiene "no soportado"

# ── V12. Idempotencia ────────────────────────────────────────
curl -X POST .../pilates/voz/perfil/whatsapp/generar
# PASS: genera nuevas plantillas sin error
# PASS: plantillas anteriores no se borran (se mantienen como histórico)
```
...
- **Cada canal = 1 LLM call.** Generar los 4 canales son ~4 calls × $0.005 = $0.02. Se ejecuta al onboarding + cada vez que cambia la posición matricial o el PCA.
- **Las plantillas NO se aplican automáticamente.** Todo queda en estado "borrador" hasta que Jesús aprueba y luego marca como "aplicado". El flujo completo del Arquitecto de Presencia en modo semi-auto (20d) enviará las plantillas por WhatsApp con instrucciones para copiar/pegar.
- **El canal `web` no se genera** porque no tiene prompt específico y el IRC es < 0.3. Se puede añadir en el futuro.
- **Las plantillas son versionadas por posición matricial.** Si la posición cambia (de E1 a E3), se regeneran los perfiles y las nuevas plantillas reflejan la nueva posición. Las anteriores se mantienen como histórico.
- **`generar_todos_los_perfiles()` filtra por IRC >= 0.3.** Solo genera perfiles para canales que merecen la pena. Facebook (IRC 0.39) entra justo. Si bajase a 0.28, se saltaría.
- **No hay migración SQL en este briefing.** La tabla `om_voz_perfil_plantilla` ya fue creada en 20a.
