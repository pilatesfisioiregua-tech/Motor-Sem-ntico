# B-ORG-F2-01: Capa Sensorial — Motor Gap + Motor Gradiente + Query Generation Inteligente

**Fecha:** 23 marzo 2026
**Prioridad:** Alta — G4 parte 1 de 3
**Modelo:** `gpt-4o` (generación creativa de queries en español)
**Dependencia:** B-ORG-BRAIN ejecutado (usa cerebro_organismo.py)

---

## QUÉ EXISTE

`buscador.py` tiene queries TEMPLATE hardcodeadas:
```python
QUERY_TEMPLATES = {
    "F1": ["estrategias retención clientes estudio pilates pequeño", ...],
    "F3": ["optimizar horarios estudio pilates ocupación baja", ...],
}
```

Problema: siempre busca lo mismo. No sabe POR QUÉ F3 es baja. No distingue gap de gradiente. No adapta queries al estado semántico del negocio.

## QUÉ CAMBIA

El buscador pasa de templates estáticos a búsqueda dirigida por el diagnóstico ACD dual (P41: numérico + semántico).

**Motor Gap:** "Tengo F3 = 0.25 porque 15/16 grupos están infrautilizados → buscar cómo otros estudios fusionan/cierran grupos"
**Motor Gradiente:** "Quiero pasar de E3 a E2 (subir Se y C) → buscar cómo otros estudios encontraron diferenciación y sistematizaron procesos"

## ARCHIVO A MODIFICAR

`src/pilates/buscador.py` — reescritura completa
...
### TEST 1: Query generation produce queries específicas (no genéricas)
```python
queries = await _generar_queries(estado_acd, señales)
assert len(queries["motor_gap"]) >= 3
# Queries deben ser específicas al contexto
for q in queries["motor_gap"]:
    assert len(q["query"]) > 30  # No "mejorar F3"
```

### TEST 2: Filtro ACD descarta resultados irrelevantes
```python
resultado = {"respuesta": "El precio del petróleo subió un 5% esta semana."}
filtrado = await _filtrar_acd(resultado, {"salud": 0.4, "sentido": 0.3, "continuidad": 0.4})
assert filtrado["descartado"] == True
```

### TEST 3: Sin Perplexity key, funciona sin crash
```python
result = await buscar_por_gaps()
assert "error" not in result or "perplexity" in result.get("error", "").lower()
```

### TEST 4: Resultados se persisten en om_voz_capa_a con metadatos
```sql
SELECT * FROM om_voz_capa_a WHERE metadata->>'motor' = 'gap' ORDER BY created_at DESC LIMIT 5;
-- Debe tener lentes_tocadas y prioridad_acd
```
