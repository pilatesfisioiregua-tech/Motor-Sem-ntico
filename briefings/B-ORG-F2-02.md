# B-ORG-F2-02 v2: Enjambre Cognitivo — Diagnóstico en Nivel 1 (Repertorio INT×P×R)

**Fecha:** 23 marzo 2026 (REESCRITO — versión anterior era incorrecta)
**Prioridad:** Alta — G4 parte 2 de 3
**Modelo:** `anthropic/claude-sonnet-4.6` (álgebra cognitiva, modelo causal 4 niveles)
**Dependencia:** B-ORG-F2-01 ejecutado

---

## POR QUÉ SE REESCRIBE

La versión anterior tenía 3 agentes de lente que medían S/Se/C + 7 agentes de función que medían F1-F7 + 6 clusters INT. Eso es diagnosticar en Nivel 3-4 (SÍNTOMAS).

Los documentos del Reactor v5 (inversion_causal_INT_P_R.md, gradientes_equilibrio_inteligencias.md, taxonomia_pensamiento_razonamiento.md) demuestran que:
...
Nivel 4 (SÍNTOMA):   Perfil funcional 7F×3L (lo observable)
```

Y las reglas IC2-IC6 formalizan cómo las combinaciones INT×P×R impulsan o frenan funciones:
- IC2: Una INT en monopolio (sin complementarias) = disfunción
- IC3: INT + P incompatible = la INT se aborta
- IC4: INT + R incompatible = la INT concluye mal
- IC5: P operan en pares complementarios (divergente+convergente)
- IC6: R se validan cruzadamente (un R aislado amplifica sesgos)
- IC7: E4 requiere ≥7 INT + ≥4 P + ≥3 R distribuidos entre 3 lentes

**El enjambre correcto diagnostica en Nivel 1 (qué INT×P×R usa el negocio) y DERIVA los niveles 2-4.**

## ARQUITECTURA CORREGIDA

```
DATOS REALES + CORPUS VIVO
         |
         v
  AGENTE DETECTOR DE REPERTORIO (Nivel 1)
  "¿Qué INT×P×R está usando este negocio? ¿Qué le falta?"
         |
         v
  AGENTE DETECTOR DE DISFUNCIONES (IC2-IC6)
  "¿Hay monopolios? ¿Desacoples INT-P? ¿Desacoples INT-R? ¿P sin complemento?"
         |
         v
  AGENTE MECANISMO CAUSAL (Nivel 2)
  "Esa configuración PRODUCE este sesgo de lentes. Por eso S=0.46, Se=0.34, C=0.40"
         |
         v
  6 AGENTES PERCEPTIVOS (clusters INT×P×R)
  Cada uno percibe desde su ángulo Y diagnostica si su combinación impulsa o frena
         |
         v
  → Todo emitido al bus como PERCEPCION_CAUSAL
  → Alimenta al Compositor (B-ORG-F2-03)
```

## ARCHIVO A CREAR

`src/pilates/enjambre.py` — REESCRITO con modelo causal
...
async def _call_agente(system_prompt: str, user_prompt: str, nombre: str) -> dict:
    ...
    try:
        ...
        log.warning("enjambre_parse_error", agente=nombre)
        return {"error": "parse_error", "raw": raw[:300] if 'raw' in dir() else ""}
...
```sql
-- 021_enjambre.sql (sin cambios respecto a versión anterior)
...
9 agentes × claude-sonnet-4.6 (~$0.05/call) = **~$0.45/ejecución**
(3 secuenciales + 6 paralelos)
Frecuencia: 1x/semana = **~$1.80/mes**

## TESTS

### TEST 1: Detector de Repertorio identifica INT×P×R con evidencias
```python
result = await ejecutar_enjambre()
rep = result["resultados"]["repertorio"]
assert "repertorio_detectado" in rep
assert len(rep["repertorio_detectado"]["INT_activas"]) >= 3
# Cada INT detectada debe tener evidencia concreta
for int_det in rep["repertorio_detectado"]["INT_activas"]:
    assert len(int_det["evidencia"]) > 20  # No vale "presente"
```

### TEST 2: Detector de Disfunciones encuentra al menos 1 disfunción
```python
disf = result["resultados"]["disfunciones"]
assert len(disf.get("disfunciones", [])) >= 1
# Cada disfunción debe tener mecanismo causal, no solo "falta X"
for d in disf["disfunciones"]:
    assert "mecanismo" in d
    assert len(d["mecanismo"]) > 30
```

### TEST 3: Mecanismo Causal explica la cadena Nivel 1→2→3→4
```python
causal = result["resultados"]["causal"]
assert "cadena_causal" in causal
assert "nivel_1_causa" in causal["cadena_causal"]
assert "nivel_4_sintomas" in causal["cadena_causal"]
```

### TEST 4: Clusters evalúan si sus herramientas impulsan o frenan
```python
for cl_id, cl_resp in result["resultados"]["clusters"].items():
    if "error" not in cl_resp:
        assert "impulsa" in cl_resp or "frena" in cl_resp
```

### TEST 5: Señales PERCEPCION_CAUSAL emitidas al bus
```sql
SELECT count(*) FROM om_bus_senales WHERE tipo='PERCEPCION_CAUSAL';
-- Debe ser >= 7
```
