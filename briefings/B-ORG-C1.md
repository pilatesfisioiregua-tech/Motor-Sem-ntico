# B-ORG-C1: Collectors — Meta Graph API + GBP + WA Analytics

**Fecha:** 23 marzo 2026
**Prioridad:** Alta (cierra G6 APRENDER con datos reales)
**Dependencia:** Credenciales Meta + GBP (Jesús debe crearlas)
**Acción:** Escribir TODO el código ahora. Se activa al añadir env vars.

---

## CONTEXTO

voz_ciclos.py tiene `escuchar()` que detecta señales, y `recalcular_irc()` que ajusta IRC.
Pero las dos dependen de datos manuales — no hay pull automático de APIs.

El ciclo APRENDER (G6) gira en vacío.

Este briefing crea los collectors que pullan datos reales de:
1. **Instagram** (via Meta Graph API) → métricas de posts, reach, engagement, demographics
2. **Google Business Profile** (via Performance API) → búsquedas, acciones, vistas
3. **WhatsApp** (via WA Business API) → analytics de conversaciones

Los datos alimentan `registrar_telemetria()` → `recalcular_irc()` → la estrategia semanal cambia automáticamente.

## ARCHIVOS A CREAR

1. `src/pilates/collectors.py` — Módulo unificado de collectors
2. Actualizar `voz_ciclos.py` → `escuchar()` llama a los collectors
3. Actualizar `cron.py` → tarea diaria incluye collect

## CREDENCIALES (env vars en fly.io)

```
# Meta Graph API (Instagram + Facebook)
META_ACCESS_TOKEN              # Token larga duración (60 días, auto-renovable)
INSTAGRAM_BUSINESS_ACCOUNT_ID  # ID cuenta IG Business

# Google Business Profile
GOOGLE_SERVICE_ACCOUNT_JSON    # JSON de service account (base64 encoded)
GBP_LOCATION_ID                # ID de la ubicación en GBP

# WhatsApp (ya definidas para envío)
WHATSAPP_TOKEN                 # Ya existe
WHATSAPP_PHONE_ID              # Ya existe
```

## CÓDIGO EXACTO — collectors.py
...
async def collect_gbp() -> dict:
    ...
    if resultado["metricas"]:
        ...
        directions = resultado["metricas"].get("business_direction_requests", 0)
...
async def collect_all() -> dict:
    ...
    activos = sum(1 for r in resultados.values()
                  if isinstance(r, dict) and r.get("status") != "skip" and "error" not in r)
...
(PyJWT para Google service account auth)

## INTEGRACIÓN EN CRON

En `cron.py`, añadir al final de `_tarea_diaria()`:
...
## INTEGRACIÓN EN VOZ_CICLOS

En `voz_ciclos.py`, al inicio de `escuchar()`, antes de Capa B:
...
Añadir tool al cockpit para ejecutar collectors bajo demanda:
...
# En TOOL_DISPATCH:
"voz_collectors": lambda args: __import__("src.pilates.collectors", fromlist=["collect_all"]).collect_all(),
```

## TESTS

### TEST 1: collect_all() sin credenciales → skip gracioso
```python
result = await collect_all()
assert result["status"] == "ok"
assert result["collectors_activos"] == 0  # Sin credenciales, todos skip
```

### TEST 2: collect_whatsapp() con datos DB
```python
# Si hay mensajes en om_mensajes_wa:
result = await collect_whatsapp()
assert "metricas" in result
assert "mensajes_recibidos" in result["metricas"]
```

### TEST 3: Telemetría se registra
```python
# Después de collect_all(), verificar om_voz_telemetria_canal:
SELECT * FROM om_voz_telemetria_canal WHERE created_at > now() - interval '1 minute';
```

### TEST 4: IRC se recalcula
```python
# Después de collect con datos, los scores IRC cambian
```

---

## NOTA SOBRE META ACCESS TOKEN

El token de Meta expira cada 60 días. Para auto-renovar:

```python
async def refresh_meta_token():
    """Renueva el token de larga duración de Meta."""
    token = os.getenv("META_ACCESS_TOKEN")
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code == 200:
            ...
            # TODO: actualizar secret en fly.io via API o notificar a Jesús
            return new_token
    return None
```

Esto se puede añadir al cron mensual para que el token nunca expire.
