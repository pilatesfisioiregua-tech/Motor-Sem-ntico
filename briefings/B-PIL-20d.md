# B-PIL-20d: 5 Ciclos + ISP Automático + Telemetría + Briefing Voz

**Fecha:** 2026-03-22
**Ejecutor:** Claude Code
**Dependencia:** B-PIL-20a + 20b + 20c desplegados (tablas om_voz_senales, om_voz_telemetria_canal, om_voz_isp, om_voz_calendario, om_voz_perfil_plantilla con datos)
**Coste:** ~$0.005/ciclo (1 LLM call para priorizar señales)

---

## CONTEXTO

Los 5 ciclos de B2.8 §3.1 (ESCUCHAR → PRIORIZAR → PROPONER → EJECUTAR → APRENDER) son el sistema nervioso del Bloque Voz. Conectan todo lo que ya existe:

- **20a** creó las tablas (`om_voz_senales`, `om_voz_telemetria_canal`, etc.)
- **20b** implementó PROPONER (Motor Tridimensional → calendario semanal)
- **20c** implementó CONFIGURAR (Arquitecto de Presencia → perfiles por canal)

Lo que falta:
- **ESCUCHAR:** Detector de señales que alimenta `om_voz_senales`
- **PRIORIZAR:** Clasificar señales por urgencia/impacto cruzando 3 capas
- **EJECUTAR:** Modo semi-auto (enviar tareas por WA a Jesús)
- **APRENDER:** Registrar telemetría + recalcular IRC + ISP automático
- **BRIEFING VOZ:** Sección Voz en el briefing semanal existente (`briefing.py`)

**Pragmatismo:** No tenemos APIs reales de Google Business, Instagram, WA Business aún. ESCUCHAR opera con: señales internas (Capa B, ya implementadas en `_recoger_senales_internas`), Capa A (Perplexity, Open-Meteo ya funcionan), y señales manuales. Cuando se conecten APIs, los detectores se enchufan sin cambiar la arquitectura.

---

## FASE D1: Motor de Ciclos

**Archivo:** Crear `src/pilates/voz_ciclos.py`
...
## FASE D2: Integrar sección Voz en briefing.py

**Archivo:** `src/pilates/briefing.py` — EDITAR función `generar_briefing()`.

Buscar el `return` final de `generar_briefing()` y ANTES de ese return, añadir:
...
Y en el dict del return, añadir la clave `"voz": seccion_voz`.

**IMPORTANTE:** La edición exacta depende de cómo termine la función `generar_briefing()`. Claude Code debe leer el final de la función, localizar el `return {...}` y añadir `"voz": seccion_voz` dentro del dict + el bloque de código antes del return.

---

## FASE D3: Endpoints

**Archivo:** `src/pilates/router.py` — AÑADIR al final, después de los endpoints de 20c.
...
```bash
# ── V1. Archivo creado ───────────────────────────────────────
ls src/pilates/voz_ciclos.py
# PASS: archivo existe

# ── V2. Importación limpia ───────────────────────────────────
python -c "from src.pilates.voz_ciclos import escuchar; print('OK')"
# PASS: sin errores

# ── V3. ESCUCHAR ─────────────────────────────────────────────
curl -X POST .../pilates/voz/ciclo/escuchar
# PASS: status="ok"
# PASS: senales_creadas >= 0 (depende del estado del negocio)

# ── V4. PRIORIZAR (señales pendientes) ───────────────────────
curl .../pilates/voz/senales
# PASS: status="ok"
# PASS: senales es array
# PASS: ordenadas por urgencia (critica > alta > media > baja)

# ── V5. Escuchar dos veces (deduplicación) ───────────────────
curl -X POST .../pilates/voz/ciclo/escuchar
# PASS: senales_creadas = 0 (duplicados filtrados por ventana 24h)

# ── V6. Marcar señal procesada ───────────────────────────────
# ID=$(curl -s .../pilates/voz/senales | jq -r '.senales[0].id')
# (solo si hay señales)
# curl -X POST .../pilates/voz/senales/$ID/procesada
# PASS: status="procesada"

# ── V7. Registrar telemetría ─────────────────────────────────
curl -X POST .../pilates/voz/telemetria \
  -H "Content-Type: application/json" \
  -d '{"canal":"whatsapp","periodo":"2026-W12","metricas":{"leads_generados":5,"reservas_directas":3,"conversiones_cliente":2,"tasa_respuesta":85}}'
# PASS: status="ok"

# ── V8. Registrar telemetría duplicada (upsert) ─────────────
curl -X POST .../pilates/voz/telemetria \
  -H "Content-Type: application/json" \
  -d '{"canal":"whatsapp","periodo":"2026-W12","metricas":{"leads_generados":7,"reservas_directas":4,"conversiones_cliente":3,"tasa_respuesta":90}}'
# PASS: status="ok" (actualiza, no duplica)

# ── V9. Recalcular IRC ──────────────────────────────────────
curl -X POST .../pilates/voz/irc/recalcular
# PASS: status="ok"
# PASS: canales_actualizados >= 1 (whatsapp tiene telemetría)

# ── V10. Verificar IRC actualizado ───────────────────────────
curl .../pilates/voz/irc
# PASS: whatsapp tasa_conversion_historica ha cambiado (ya no es 0.8 del seed)

# ── V11. ISP automático ─────────────────────────────────────
curl .../pilates/voz/isp
# PASS: status="ok"
# PASS: isp_global es número entre 0 y 100
# PASS: canales tiene whatsapp, google_business, instagram, facebook
# PASS: cada canal tiene isp_score, elementos, ok, pendientes

# ── V12. Ciclo completo ─────────────────────────────────────
curl -X POST .../pilates/voz/ciclo/completo
# PASS: status="ok"
# PASS: ciclos.escuchar tiene senales_creadas
# PASS: ciclos.priorizar tiene total, criticas, altas
# PASS: ciclos.proponer tiene semana y contadores
# PASS: ciclos.ejecutar tiene modo="manual"
# PASS: ciclos.aprender tiene irc_actualizados e isp_global

# ── V13. Briefing incluye sección Voz ────────────────────────
curl .../pilates/briefing
# PASS: respuesta incluye clave "voz"
# PASS: voz.estrategia tiene foco y narrativa
# PASS: voz.isp tiene global y canales
# PASS: voz.senales tiene total
```
...
- **ESCUCHAR es extensible.** Cuando se conecten APIs reales (GBP, IG Graph, WA Business), se añaden detectores en `escuchar()` sin cambiar nada más. Cada detector es un bloque independiente que registra señales en `om_voz_senales`.
- **EJECUTAR es manual.** Sin APIs de canales, la ejecución es: Jesús ve el calendario → aprueba → publica él mismo. 20e conectará esto al cockpit. Cuando haya APIs, el modo semi-auto enviará contenido aprobado directamente.
- **IRC se recalcula con datos reales.** La primera vez que registras telemetría de WA con conversiones, el IRC de WA cambia. Esto es el ciclo APRENDER en acción: datos reales reemplazan estimaciones.
- **ISP automático reemplaza el checklist manual de voz.py.** Ya no hay que marcar elementos a mano — el sistema evalúa plantillas, calendario, telemetría y estrategia automáticamente.
- **La sección Voz del briefing es tolerante a errores.** Si algo falla (tabla vacía, módulo no importable), el briefing funciona igual — solo sin la sección Voz.
- **Deduplicación de señales:** `_registrar_senal()` evita duplicados del mismo tipo en 24h. Si escuchas el lunes y el martes, solo crea señales nuevas si la situación cambió.
- **El ciclo completo (~2 seg sin LLM):** ESCUCHAR + PRIORIZAR + ISP + IRC son operaciones de DB. No llaman a LLM. El único LLM call es `calcular_estrategia()` de 20b, que se ejecuta por separado.
