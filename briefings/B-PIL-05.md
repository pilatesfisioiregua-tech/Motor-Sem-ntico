# B-PIL-05: Automatismos — Triggers + Cron + Lógica de Negocio Automática

**Fecha:** 2026-03-20
**Ejecutor:** Claude Code
**Dependencia:** B-PIL-04 (endpoints sesiones/cargos/pagos)
**Coste:** $0
...
Los endpoints CRUD y de lógica de negocio están montados (B-PIL-03/04). Ahora necesitamos los automatismos que eliminan trabajo manual. Fuente: Exocortex v2.1 S5, S13, S16.

Hay dos tipos:
1. **Triggers inline** — lógica que se ejecuta dentro de endpoints existentes (ya parcialmente en B-PIL-04)
2. **Cron/batch** — lógica que se ejecuta periódicamente o bajo demanda

Todo vive en `src/pilates/automatismos.py` como funciones async invocables desde endpoints.

---

## FASE A: Crear `src/pilates/automatismos.py`
...
async def ejecutar_cron(tipo: str) -> dict:
    ...
    if tipo == "inicio_semana":
        ...
        # Importar función de suscripciones del router
        from src.pilates.router import TENANT
        pool = await _get_pool()
...
## FASE B: Añadir endpoints cron a router.py
...
## FASE C: Integrar side-effects en endpoints existentes

**Archivo:** `@project/src/pilates/router.py`

En el endpoint `completar_sesion` (POST /sesiones/{id}/completar), AÑADIR al final antes del return:
...
En el endpoint `marcar_grupo` (POST /sesiones/{id}/marcar-grupo), AÑADIR al final antes del return:
...
- `src/pilates/automatismos.py` creado con 6 funciones
- POST /pilates/cron/inicio_semana genera sesiones + alertas
- POST /pilates/cron/inicio_mes genera suscripciones + días esperados
- POST /pilates/cron/generar-sesiones crea sesiones para la semana (idempotente)
- GET /pilates/alertas devuelve alertas retención
- POST /pilates/bizum-entrante busca cliente por teléfono + concilia FIFO
- Completar sesión y marcar-grupo actualizan días_esperados como side-effect
- Todos los crons son idempotentes (re-ejecutar = sin duplicados)

---

## RESUMEN AUTOMATISMOS IMPLEMENTADOS

| # | Automatismo | Trigger | Endpoint |
|---|-------------|---------|----------|
| 1 | Generar sesiones semana | Cron lunes (manual MVP) | POST /cron/inicio_semana |
| 2 | Suscripciones mensuales | Cron día 1 | POST /cron/inicio_mes |
| 3 | Calcular días esperados | Cron día 1 | POST /cron/inicio_mes |
| 4 | Alertas retención | Cron semanal | GET /alertas |
| 5 | Bizum entrante | Manual (tecla B) | POST /bizum-entrante |
| 6 | Actualizar asistencia/mes | Side-effect completar/marcar | Automático |
| 7 | Conciliación FIFO | Dentro de POST /pagos | Ya en B-PIL-04 |
| 8 | Cancelación tardía | Dentro de POST /marcar | Ya en B-PIL-04 |
| 9 | Método pago aprendido | Dentro de POST /pagos | Ya en B-PIL-04 |

Los crons en MVP se ejecutan manualmente (Jesús pulsa un botón o usa curl). En v1 se automatizarán con fly.io scheduled machines o cron interno.
