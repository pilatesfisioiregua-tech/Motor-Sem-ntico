"""motor.pensar() — API única de pensamiento OMNI-MIND (P61).

Sandwich LLM:
  1. código_pre: selección modelo + presupuesto + caché ($0)
  2. LLM: OpenRouter ($$)
  3. código_post: verificación IC + telemetría ($0)

TODO pensamiento del sistema pasa por aquí.
Stateless: no guarda estado entre llamadas (el estado vive en organismo/).
"""
...
class ConfigPensamiento:
    ...
    lente: Optional[str] = None         # salud, sentido, continuidad
    ...
    timeout: float = 90.0
    verificar_output: bool = True       # Sandwich: verificar IC post-LLM
...
class Pensamiento:
    ...
    verificacion_score: float = 1.0
    verificacion_warnings: list[str] = field(default_factory=list)
...
async def pensar(
    ...
    Sandwich LLM:
      1. Selecciona modelo (pizarra modelos → fallback chain)
      2. Verifica presupuesto
      3. Busca en caché semántico
      4. Llama a OpenRouter
      5. Verificación IC post-LLM (kernel puro $0)
      6. Guarda caché + telemetría
    """
    ...
    if _presupuesto_ciclo >= PRESUPUESTO_MAX_SEMANAL:
        ...
        log.warning("motor.presupuesto_excedido",
                    gastado=_presupuesto_ciclo, max=PRESUPUESTO_MAX_SEMANAL)

    # 3. Caché semántico
    if config.usar_cache:
        cached = await _buscar_cache(system, user, modelo, config.tenant_id)
        if cached:
            log.info("motor.cache_hit", funcion=config.funcion, modelo=modelo)
            return Pensamiento(
    ...
    # 5. Verificación IC post-LLM (sandwich: código puro $0)
    if config.verificar_output:
        v = verificar(
            pensamiento.texto,
            funcion=config.funcion,
        )
        pensamiento.verificacion_score = v.score
        pensamiento.verificacion_warnings = v.warnings
        if not v.ok:
            log.warning("motor.verificacion_fallida",
                        score=v.score, errors=v.errors, warnings=v.warnings)

    # 6. Presupuesto
    _presupuesto_ciclo += pensamiento.coste_usd

    # 7. Caché + telemetría (fire-and-forget)
    if config.usar_cache and not pensamiento.cache_hit:
        try:
            await _guardar_cache(system, user, modelo, pensamiento.texto, config)
        except Exception as e:
            log.debug("motor.cache_save_error", error=str(e)[:80])

    try:
        await _registrar_telemetria(pensamiento, config)
    except Exception as e:
        log.debug("motor.telemetria_error", error=str(e)[:80])

    log.info("motor.pensar_ok",
             ...
             verificacion=round(pensamiento.verificacion_score, 2),
             tiempo_ms=tiempo_ms)
...
async def _seleccionar_modelo(config: ConfigPensamiento) -> str:
    ...
    try:
        ...
        return _MODELOS_DEFAULT.get(config.complejidad, "openai/gpt-4o")
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise
                log.warning("motor.retry", modelo=modelo, attempt=attempt,
                            error=str(e)[:80])
                import asyncio
...
async def _buscar_cache(system: str, user: str, modelo: str,
...
async def _registrar_telemetria(pensamiento: Pensamiento,
                                 config: ConfigPensamiento):
    from src.db.client import get_pool
